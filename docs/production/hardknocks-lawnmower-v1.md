...
- Engaged views: 634 · Unique viewers: 579 · Watch time: 5.6h · Average view duration: 0:30
- Audience retention (lifetime): Stayed to watch **54.1%**, AVD 0:30
- How viewers engaged (since published): Stayed to watch **54.1%** / Swiped away **45.9%**

This confirms the API's `averageViewPercentage` (59.84%) does NOT match Studio's real "Stayed to
watch" (54.1%) — the API reads +5.7pp higher, the same direction as the +6.0pp gap found on
Dangote in ADR-0025's addendum (now a 2nd confirmed case, see ADR-0025's 2026-07-13 addendum).
`EXPERIMENT-LOG.md`'s `Stayed (Retention/Overall)` / `Stayed (Engagement/Hook)` / `AVD` columns for
this video have been corrected to the real Studio numbers (54.1% / 54.1% / 0:30); the auto-fetched
API values above are kept as a record of what automation returned, not as the authoritative
number.
