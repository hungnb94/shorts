...
`output/projects/aiwork/final/Mw7jeR6R6iE-retention.csv`._

- **Views**: 396 · **AVD**: 0:17 (36.74% of 46.2s) · **CTR**: not available via API (confirmed, see ADR-0025)
- **Key moments** (z-score vs this video's own smoothed baseline):
  - peak at ~4.16s (6.4%, z=4.52)
  - peak at ~0.92s (3.9%, z=3.17)
  - dip at ~10.63s (-7.7%, z=-2.8)

### Studio Cross-Check (manual, 2026-07-13)
Real YouTube Studio read (Engagement tab, screenshot provided by user):
- Engaged views: 426 · Unique viewers: 329 · Watch time: 2.6h · Average view duration: 0:20
- Audience retention (lifetime): Stayed to watch **43.6%**, AVD 0:20
- How viewers engaged (since published): Stayed to watch **43.7%** / Swiped away **56.3%**

This time the API's `averageViewPercentage` (36.74%) read **6.86 points LOWER** than Studio's real
"Stayed to watch" (43.6%) — the opposite direction from Dangote (API +6.0pp higher) and
`dHDpDXSIAkA` (API +5.74pp higher). An earlier version of this section guessed the real number
was "likely closer to ~31-32%," assuming a fixed +5-6pp bias — that guess is now disproven and
retracted; the real number (43.6%) is *higher* than the API value, not lower.

Root-cause check: read `src/platforms/youtube-analytics.ts` end-to-end — the query is correctly
scoped (`dimensions=video`, `filters=video==<id>`, metrics `views,averageViewDuration,
averageViewPercentage`, a `startDate` well before any video's upload), so this is not a parameter
bug in this project's code. Per Google's official metric docs
(developers.google.com/youtube/analytics/metrics), `averageViewDuration`/`averageViewPercentage`
are generic long-form-video metrics that have excluded looping-clip traffic since 2021-12-13 —
they are simply a different metric family from whatever proprietary calculation YouTube Studio's
Shorts-specific "Stayed to watch" panel uses internally (same root cause ADR-0025 already
identified for `Stayed (Engagement/Hook)`/`Swiped Away` — now confirmed to extend to
`Stayed (Retention/Overall)` too, per ADR-0025's 2026-07-13 2nd addendum). There is no reliable
fixed correction factor; `EXPERIMENT-LOG.md`'s columns for this video have been corrected to the
real Studio numbers (43.7% / 43.6% / 0:20 / 56.3%).
