...
`aiwork_v3` shipped with HOOK → ESCALATE → COMPLICATION → REINFORCE → CLOSE. User feedback after watching it: *"phần nội dung gần cuối video không hợp lý lắm xong rồi ngay lập tức nhảy sang cảm ơn"* (the content near the end doesn't make sense, then it jumps straight to "thank you"). Confirmed via frame/transcript review: REINFORCE's empathy beat ("EVEN THE PEOPLE WHO BUILD THIS STUFF") led directly into CLOSE's "thank you for spending your day..." sign-off — the HEIT "Teach" stage (the actual resolution/lesson) was missing entirely. v3 stated the problem (400 lines) and showed proof it's common, but never stated what fixed it, even though the chart's "15 lines" side was already on screen.
...
No new workflow gap found. This production's root cause — "the HEIT Teach stage was missing" — is not a new failure mode this repo lacks a rule for; HEIT (Hook → Explain → Illustrate → Teach) is already the mandated script structure (AGENTS.md Key Conventions), and the gap here was an execution miss during v3's grilling session (the plan's clip table covered HOOK/ESCALATE/COMPLICATION/REINFORCE but never explicitly checked "is there a Teach beat"), not a missing rule. No ADR or WORKFLOW.md change needed. If a future video's grilling session wants a forcing function for this, the fix would be procedural (e.g., a checklist item "does the clip table include a Teach/resolution beat before CLOSE?") — noted here as a candidate but not added speculatively without a second occurrence.
