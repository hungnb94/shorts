...
## Addendum (2026-07-13): "First overlay ≤t=2s" is not sufficient — full-screen overlays must never replace the face anywhere in 0-10s

Root-caused `dF0Rr2C0Msc` (Giannis V2, "A Ferrari Costs $1.1M", **37.1% stayed — worst on record, flagged in `docs/research/hook-benchmarks-2026-07/REPORT.md` as a recommended-but-never-done forensic target**) while researching general swipe-away reduction. Direct frame reads of the actual uploaded render (`output/projects/giannis/final/2026-07-09-V1_ferrari_1.1M.mp4`, confirmed via retention-CSV duration match, not the similarly-named `V2_60pct_broke.mp4`) show the face-check at t=0/0.5/2.0s passed correctly (real Giannis interview footage, verified in `output/projects/giannis/source/faces/V1_ferrari_414_t*.jpg`) — but the video's own render script, `pipeline/giannis/render_giannis_v1.py`, then composites **full-screen** (`scale=1080:1920` + `overlay=x=0:y=0`, the same canvas size as the source, not a corner insert) Pexels b-roll clips at `"pexels": [(1, 2, "watch"), (5, 2, "art"), (8, 2, "house"), ...]` — meaning the face is completely replaced by a static stock clip for **6 of the first 10 seconds, in 3 separate 2-second blackouts** (t=1-3, 5-7, 8-10). The retention curve (`dF0Rr2C0Msc-retention.csv`) confirms the mechanism exactly: relative retention holds near its peak (1.09-1.26x average) through t≈4.5s, then falls off a cliff (1.09 → 0.86 → 0.75) precisely across t=5-9s — the 2nd and 3rd face-replacing overlays.

This is a distinct failure mode from the one this ADR originally documented (a face-less *start*): here, frame 0 was fine, and the existing rule 4 ("first overlay must land ≤t=2s") was technically satisfied (first overlay landed at t=1s) — but rule 4 implicitly assumed "overlay" meant a small supplementary graphic (data-viz callout, emoji, corner insert) layered *over* a still-visible speaker, not a full-canvas substitution that hides the face entirely. The same `overlay=x=0:y=0` full-screen pattern also appears in `pipeline/bacsihai/render_bacsihai_v3.py` (one instance literally at `(0, 3, "scale")` — replacing frame 0 itself) and `render_bacsihai_v4.py`; **it is absent from every current script** (`bacsihai_v5/v6/v7`, all `hardknocks_v*`, all `aiwork_v*`), which instead use ADR-0016's zoompan + small corner overlays — so this failure mode has already been abandoned in practice, but was never explicitly named or banned, meaning nothing stops it from being reintroduced.

**Rule extension**: within the entire 0-10s hook window (not just t=0), any Pexels/value-add overlay must be a partial/corner composite that keeps the speaker's face visible in the same frame — never a full-canvas `overlay=x=0:y=0` (or equivalent) that occupies 100% of the 1080x1920 canvas and removes the face from view, regardless of how briefly (2s already measured as enough to crater retention, repeated 2-3x compounds it). This does not ban full-screen B-roll everywhere — only inside the hook window; the project's standing "3-source combo" convention (AGENTS.md) for Pexels b-roll as visual variety remains valid for the body of a video, past t≈10s.
