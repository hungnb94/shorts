...
```bash
python3 .claude/skills/viral-video-analysis/scripts/extract_keyframes.py \
...
```bash
python3 .claude/skills/viral-video-analysis/scripts/analyze_audio.py \
  docs/research/<slug>/<id>/source.mp4 \
...
- `scripts/extract_keyframes.py` — ffprobe + ffmpeg scene-detect, writes frames + manifest.json.
  No vision API call; Claude reads frames directly (see ADR-0023 for why this replaced the
