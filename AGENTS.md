...
- **Canonical narrator** — every new English video and every material revision that regenerates TTS uses the profile selected by `data/narrator-voices/default.json` (`ronald_wayne_zack_style_qwen`); it is internal-only until explicit narrator consent/licensing clears the public-person conditioning reference (ADR-0039). Never silently fall back to Edge TTS.
- **Codex review loop** — after exact-final QC, request a structured Codex media review, apply timestamp-supported findings, rerender and repeat QC/review until the internal editorial score is at least 98 or an external blocker is recorded. The score is a quality gate, not a virality guarantee; maximum five full rerender rounds per material revision (ADR-0040).
