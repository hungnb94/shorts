#!/usr/bin/env python3
"""Render 3 Giannis Shorts v2 — verbatim subtitle sync from transcript.json."""
...
# === Load transcript ===
TRANS = json.loads(Path("output/NSeL8fFz5hw/transcript.json").read_text())["segments"]
def get_subs(src_start, src_end):
    """Return [(offset_s, offset_e, text)] for a source time range, offsets relative to src_start."""
    out = []
    for s in TRANS:
...
VARIANTS = [
    ...
    },
    {
        ...
        "pexels": [(3,2,"suit"),(10,2,"office"),(36,2,"contract"),(42,2,"books")],
    },
]
...
def render(v):
    ...
    # 1. Extract contiguous segment (raw video + audio, center-crop to 1080x1920)
    raw = d / "raw.mp4"
    src_dur = v["src_end"] - v["src_start"]
    cmd = ["ffmpeg","-y","-ss",str(v["src_start"]),"-i",str(SRC),"-t",str(src_dur),
           "-vf","scale=-2:1920,crop=1080:1920:(in_w-1080)/2:0,fps=30",
           "-c:v","libx264","-crf","18","-preset","fast","-pix_fmt","yuv420p","-r","30",
           "-c:a","aac","-b:a","192k","-ar","48000","-ac","2",str(raw)]
    r = subprocess.run(cmd,capture_output=True,text=True,timeout=120)
    if r.returncode != 0: print(f"  RAW ERROR: {r.stderr[-200:]}"); return False
    raw_dur = dur(raw); td = raw_dur; cs = max(td-5,0)
    print(f"  Segment: {raw_dur:.1f}s")
    ...
    # 3. Get verbatim subs from transcript
    subs = get_subs(v["src_start"], v["src_end"])
    print(f"  Subtitle lines: {len(subs)}")
    ...
    fv = d/"fv.mp4"
    cmd2 = ["ffmpeg","-y"]+inputs+["-filter_complex",fc,"-map",f"[{final}]","-c:v","libx264","-crf","20","-preset","fast","-an",str(fv)]
    r = subprocess.run(cmd2,capture_output=True,text=True,timeout=600)
    if r.returncode != 0: print(f"  FILTER ERROR: {r.stderr[-300:]}"); return False
    
    out = O/f"{vid}.mp4"
    cmd3 = ["ffmpeg","-y","-i",str(fv),"-i",str(raw),"-map","0:v","-map","1:a","-c:v","copy","-c:a","aac","-b:a","192k","-shortest",str(out)]
    r = subprocess.run(cmd3,capture_output=True,text=True,timeout=300)
    if r.returncode != 0: print(f"  MUX ERROR: {r.stderr[-300:]}"); return False
...
def main():
    print("="*60+"\n  GIANNIS — 3 SHORTS v2 (verbatim subs from transcript)\n"+"="*60)
    results = [render(v) for v in VARIANTS]
    print(f"\n{'='*60}\n  {sum(results)}/{len(results)} OK\n{'='*60}")
    for v,ok in zip(VARIANTS,results):
        p=O/f"{v['id']}.mp4"; d_=dur(p) if ok else 0; mb_=p.stat().st_size/1048576 if ok else 0
