#!/usr/bin/env python3
"""Generate canonical internal-only Qwen narration for HardKnocks V29."""
...
WORK = ROOT / "output" / "projects" / "hardknocks" / "clips" / "v29_supply_chain_trap_work" / "audio" / "qwen-zack"
RAW = WORK / "raw"
...
CLIPS = (
    Clip("twenty_billion_question", "So why bet twenty billion dollars on one refinery?", 4.5),
    Clip("backward_integration", "This is backward integration: build the supplier that can choke you.", 4.5),
    Clip("control_prize", "You gain control of timing, quality, and eventually cost.", 4.5),
    Clip("nameplate_capacity", "Its nameplate capacity is six hundred fifty thousand barrels a day.", 6.0),
    Clip("risk_moved", "But the refinery did not delete dependency. It moved it upstream.", 4.5),
    Clip("triple_cta", "Would you build it? Like, subscribe, and comment: control or trap?", 4.5),
    Clip("allocation_shortfall", "In early twenty twenty-six, local refineries received just forty-six percent of allocated crude.", 6.0),
    Clip("cargoes", "Dangote still said it needed thirteen to fifteen cargoes a month, and imported the rest.", 6.0),
    Clip("payoff", "Vertical integration moves the bottleneck. And every link you own makes the bet bigger.", 6.0),
    Clip("loop_question", "So, would you build your supplier?", 4.5),
)
...
def main() -> None:
    ...
    base_seed = int(profile["generation"]["seed"]) + 2900
        ...
        json.dumps(
            {
                ...
                "settings": settings,
                "seed_policy": f"{base_seed} + stable V29 clip index",
                "segments": rows,
            },
            indent=2,
        ) + "\n",
