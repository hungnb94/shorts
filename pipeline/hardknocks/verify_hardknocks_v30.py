#!/usr/bin/env python3
"""Exact-final automated media verification for HardKnocks V30."""
...
RENDERER_PATH = ROOT / "pipeline" / "hardknocks" / "render_hardknocks_v30_one_supplier_rule.py"
_spec = importlib.util.spec_from_file_location("render_hardknocks_v30_one_supplier_rule", RENDERER_PATH)
...
def main() -> None:
    ...
    for item in source_items:
        ...
        for label, at in (("start", start + 0.15), ("mid", (start + end) / 2), ("end", end - 0.15)):
            ...
            sweep_index += 1
    make_contact_sheet(str(SOURCE_SWEEP_FRAMES / "*.jpg"), CHECKS / "source-sweep.jpg", cols=3, rows=4)

    critical_times = (0.1, 1.6, 3.1, 6.1, 9.1, 13.6, 16.6, 19.6, 22.6, 25.6, 28.6, 31.6, 34.6, 37.8, 39.1, 41.6, 42.1, 45.1, 48.1, 51.1, 54.1, 57.1)
    for index, at in enumerate(critical_times):
        extract_frame(at, CRITICAL_FRAMES / f"{index:02d}-{at:05.1f}.jpg", width=540, height=960)
    make_contact_sheet(str(CRITICAL_FRAMES / "*.jpg"), CHECKS / "critical-frames.jpg", cols=6, rows=4, width=180, height=320)
    ...
    required_patterns = {
        "one_supplier": r"one supplier",
        "shutdown_business": r"shut down.*business|business.*shut down",
        "dangote": r"dangot[ei]",
        "twenty_billion": r"twenty billion|20 billion",
        "nigeria_oil": r"nigeria.*oil|oil.*nigeria",
        "other_countries": r"other countries",
        "fuel": r"\bfuel\b",
        "refinery": r"\brefinery\b",
        "crude_daily": r"crude every day|crude.*daily",
        "everything_stops": r"everything stops",
        "cta_comment": r"\bcomment\b",
        "cta_follow": r"\bfollow\b",
        "forty_six": r"forty six percent|46 percent|46%",
        "allocated_crude": r"allocated crude",
        "problem_moved": r"problem.*moved|moved.*problem",
        "fuel_to_crude": r"fuel.*crude",
        "protect_input": r"protect.*input|input.*protect",
        "cannot_run_without": r"cannot run without|can t run without",
        "own_it": r"own it",
        "contract_it": r"contract it",
        "backup": r"\bbackup\b",
    }
    token_checks = {name: re.search(pattern, asr_text) is not None for name, pattern in required_patterns.items()}
    hook_asr = word_window(asr, 0, 7.5)
    cta_asr = word_window(asr, 37.5, 42.0)
    tail_asr = word_window(asr, 42.0, 58.5)
    ...
    checks = {
        ...
        "duration_exact_58_5": abs(final_duration - 58.5) <= 0.08,
        ...
        "frame_count_1755": int(video.get("nb_read_frames", 0)) == 1755,
        "full_decode": decode.returncode == 0 and not decode.stderr.strip(),
        "beat_count_39": len(beats) == 39,
        ...
        "caption_by_0_2": renderer.CAPTIONS["hook_problem"][0][0] <= 0.2,
        "hook_plain_language": (
            renderer.CAPTIONS["hook_problem"][0][2] == "ONE SUPPLIER"
            and renderer.CAPTIONS["hook_problem"][1][2] == "CAN STOP YOUR BUSINESS"
        ),
        "burned_stock_labels_absent": all(
            not item.get("illustration_label")
            for item in timeline
            if item.get("kind") == "narration_stock"
        ),
        ...
        "hook_stock_motion_gate": median_hook_delta > 1 and not sustained_frozen_hook,
        ...
        "targeted_hook_asr": "one supplier" in hook_asr and "shut down" in hook_asr and ("twenty billion" in hook_asr or "20 billion" in hook_asr),
        "targeted_cta_asr": all(token in cta_asr for token in ("supplier", "stop", "comment", "follow")),
        "targeted_tail_asr": all(token in tail_asr for token in ("protect", "input", "backup", "one supplier", "shut down")),
