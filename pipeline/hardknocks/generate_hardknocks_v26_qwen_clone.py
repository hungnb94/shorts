#!/usr/bin/env python3
"""Generate canonical internal-only Qwen narration for HardKnocks V26."""
...
WORK = ROOT / "output" / "projects" / "hardknocks" / "clips" / "v26_rejected_ten_percent_work" / "audio" / "qwen-zack"
RAW = WORK / "raw"
...
CLIPS = (
    Clip(
        "casino_misdirect",
        "It sounded like an online casino. It wasn't.",
        3.00,
    ),
    Clip(
        "distribution_model",
        "He sold games to operators. One product could spread through thousands of casino storefronts.",
        4.80,
    ),
    Clip(
        "valuation_math",
        "At his reported five-billion valuation, ten percent equals five hundred million before dilution.",
        5.40,
    ),
    Clip(
        "triple_cta",
        "Invest or pass? Like, subscribe, and comment yes or no.",
        3.50,
    ),
    Clip(
        "valuation_guardrail",
        "That is headline math, not a verified return. Private valuations change, and ownership dilutes across funding rounds.",
        7.20,
    ),
    Clip(
        "aws_independent_check",
        "AWS documented Spribe at forty-five hundred casinos in one hundred countries, with thirty-five million monthly players.",
        7.20,
    ),
    Clip(
        "distribution_payoff",
        "The bet investors missed wasn't the casino. It was one supplier reaching thousands of storefronts.",
        5.60,
    ),
)
...
def main() -> None:
    ...
    base_seed = int(profile["generation"]["seed"]) + 2600
        ...
        json.dumps(
            {
                ...
                "settings": settings,
                "seed_policy": f"{base_seed} + stable V26 clip index",
                "segments": rows,
            },
            indent=2,
        )
