#!/usr/bin/env python3
"""Generate deterministic Qwen narration segments for paperclip_v1.

The script loads the pinned Finance narrator once, verifies model/reference/system
provenance, caches raw 24 kHz mono output, and fits only overlong lines with
Rubber Band. Fitted output is 48 kHz stereo PCM and exactly matches each semantic
slot. It never hard-trims speech to make a slot.
"""
...
def synthesize(
    ...
    if len(sample_rates) != 1:
        raise RuntimeError(f"inconsistent Qwen sample rates: {sample_rates}")
...
def main() -> None:
    ...
    snapshot, model_evidence = verified_model_snapshot(
        profile["models"]["voice_clone"]
    )
    ...
    for index, segment in enumerate(config["segments"]):
        ...
        if cache_hit:
            ...
            audio, sample_rate, synthesis_seconds = synthesize(
                model, reference, segment["text"], seed, profile
            )
            sf.write(raw, audio, sample_rate, subtype="PCM_16")
            cache_meta.write_text(
                json.dumps(cache_key, indent=2) + "\n", encoding="utf-8"
            )
            raw_seconds = len(audio) / sample_rate

        slot = float(segment["slot_end"] - segment["slot_start"])
        speed = max(1.0, raw_seconds / max(slot - 0.06, 0.1))
        if raw_seconds > slot * 2.0:
            raise RuntimeError(
                f"{segment['id']} pathological duration "
                f"{raw_seconds:.3f}s for {slot:.3f}s slot"
            )
        if speed > max_post_tempo:
            ...
            print(json.dumps(violation, ensure_ascii=False), flush=True)
            continue
    ...
    report = {
        ...
        "settings": settings,
        "seed_policy": f"{base_seed} + stable narration segment index",
    ...
    if violations:
        raise RuntimeError(
            ...
            + ", ".join(
                f"{item['id']}={item['required_atempo']:.4f}x"
