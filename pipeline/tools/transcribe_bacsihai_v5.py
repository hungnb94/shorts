...
audio = sys.argv[1] if len(sys.argv) > 1 else "output/projects/bacsihai/source/AGBrXy-2SxI_audio16k.wav"
model = sys.argv[2] if len(sys.argv) > 2 else "mlx-community/whisper-medium-mlx"
out = sys.argv[3] if len(sys.argv) > 3 else "output/projects/bacsihai/source/AGBrXy-2SxI_transcript.json"
